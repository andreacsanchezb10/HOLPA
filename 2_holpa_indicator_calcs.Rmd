...
country_short = "sen"
country.data.path = "C:/Users/sjones/CGIAR/Sanchez, Andrea Cecilia (Alliance Bioversity-CIAT) - HOLPA_data/Senegal/senegal_data_clean/sen/"
...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  mutate(nut_chem_area_ha = ifelse(nut_chem_area_unit %in% c("acres","in acres","Acres"), nut_chem_area * acre_to_hectare, nut_chem_area),
         nut_orgF_area_ha = ifelse(nut_orgF_area_unit %in% c("acres","in acres","Acres"), nut_orgF_area * acre_to_hectare, nut_orgF_area),
         nut_org_area_ha = ifelse(nut_org_area_unit %in% c("acres","in acres","Acres"), nut_org_area * acre_to_hectare, nut_org_area)) %>%
...
ggsave(paste0(global.data.path,"figures/kpi boxplots ",country,".tif"),width=7,height=7,units="in")
...
indicator_scaling(kpi_scores, ind="kpi1_crop_health",method="none") # CHECK sub-indicator label - should be avoided so 100 is positive?
indicator_scaling(kpi_scores, ind="kpi1b_crop_health_fieldwork",method="none") # CHECK
...
indicator_scaling(kpi_scores, ind="kpi6b_abd_concentration_index", method="none") 
...
indicator_scaling(kpi_scores, ind="kpi15_diet_diversity", method="set",min=0,max=10) # CHECK
