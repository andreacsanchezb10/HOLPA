...
d <- d %>% full_join(temp_kpi_calc) %>%
  mutate(indicator = ifelse(subindicator=="soil_health_overall","kpi3_soil_health",indicator),
         subindicator_label = ifelse(subindicator=="soil_health_overall","Perceived soil erosion and fertility from 1 (infertile and major soil erosion) to 3 (highly fertile with no soil erosion)",subindicator_label),
...
d <- d %>%   mutate(indicator = ifelse(subindicator=="land_security_perceived_weighted","kpi17_land_tenure",indicator),
         subindicator_label = ifelse(indicator=="kpi17_land_tenure","Perceived land tenure security from 1 (Not at all likely to lose access to any land) to 5 (Extremely likely to lose access to all land)",subindicator_label),
         indicator_order = ifelse(indicator =="kpi17_land_tenure",17.1,indicator_order)) %>%
  
  mutate(indicator = ifelse(subindicator=="land_owned_share","kpi17b_land_tenure",indicator),
         subindicator_label = ifelse(indicator=="kpi17b_land_tenure","Share of household land that is owned (%)",subindicator_label),
         indicator_order = ifelse(indicator =="kpi17b_land_tenure",17.2,indicator_order))
...
d <- d %>% full_join(temp_kpi_calc) %>%
  mutate(indicator = ifelse(subindicator=="wellbeing_average","kpi18_human_wellbeing",indicator),
         subindicator_label = ifelse(indicator=="kpi18_human_wellbeing","Perceived life satisfaction from 1 (Completely dissatisfied) to 5 (Completely satisfied)",subindicator_label),
         indicator_order = ifelse(indicator =="kpi18_human_wellbeing",18,indicator_order))
