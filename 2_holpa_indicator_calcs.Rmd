...
indicator_scaling <- function(data, ind, method, min,max){
  ...
  data <- data %>% left_join(data_subset,by=c("kobo_farmer_id","indicator"),keep=NULL) ### PROBLEM WITH THIS LINE
}

data_output <- indicator_scaling(data = d, ind="kpi1_crop_health",method="none")
data_output <- indicator_scaling(data = data_output, ind="kpi2_animal_health",method="none")
data_output <- indicator_scaling(data = data_output, ind="kpi3_soil_health",method="set",min=1,max=5)
data_output <- indicator_scaling(data = data_output, ind="kpi4_nutrient_use", method="data_driven", min=NA,max=NA)
data_output <- indicator_scaling(data = data_output, ind="kpi18_human_wellbeing", method="set",min=1, max=5)
...
ggplot(data_output %>% filter(!is.na(score_scaled)),
       aes(x=reorder(indicator,indicator_order),y=score_scaled,colour=theme))+
  geom_boxplot()+
  #coord_polar()+
