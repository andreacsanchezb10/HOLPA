...
temp_kpi_calc <- d %>%
  dcast(kobo_farmer_id+country+module+theme+indicator+name_question+label_choice~,value.var="name_choice")


### Performance KPI6a biodiversity - abd species richness/ha DROP? ####
...
temp_kpi_calc <- d %>% filter(!(module == "agroecology")) %>% 
  filter(subindicator %in% c("abd_crop_richness", "abd_crop_local","abd_crop_area", "abd_livestock_local", "abd_livestock_richness","abd_livestock_area_owned","abd_livestock_area_shared", "abd_fish_richness","abd_fish_local","abd_fish_area")) %>%
 dcast(kobo_farmer_id+country+indicator ~subindicator,value.var="score") %>%
  
  # WHEN FISH PRESENT, add: "abd_fish_richness","abd_fish_local","abd_fish_area")
  mutate_at(c("abd_crop_richness", "abd_crop_local","abd_crop_area", "abd_livestock_local","abd_livestock_richness","abd_livestock_area_owned","abd_livestock_area_shared"), ~ifelse(is.na(.),0,as.numeric(.))) %>%

  mutate(abd_crop_area_unit = area_unit_crops,
         #abd_fish_area_unit = area_unit_fish,
         abd_livestock_area_unit = area_unit_livestock) %>%
  
  mutate(abd_crop_species_per_ha = ifelse(abd_crop_richness>0 & abd_crop_area_unit %in% c("acre","Acre","acres","Acres"), abd_crop_richness/(abd_crop_area*acre_to_hectare),ifelse(abd_crop_richness>0, abd_crop_richness/abd_crop_area,NA))) %>%
  
  mutate(abd_crop_species_per_ha = ifelse(abd_crop_species_per_ha == Inf,NA,abd_crop_species_per_ha)) %>%
  
#  mutate(abd_fish_species_per_ha = ifelse(abd_fish_richness>0 & #abd_livestock_area_unit %in% c("acre","Acre","acres","Acres"), #abd_fish_richness/(abd_fish_area*acre_to_hectare), ifelse(abd_fish_richness>0, #abd_fish_richness/abd_fish_area,NA))) %>%
  
  mutate(abd_crop_species_per_ha = as.numeric(abd_crop_species_per_ha),
         indicator = "kpi6a_abd_species_diversity",
         subindicator = "abd_species_diversity",
         subindicator_label = "Crop diversity from 1 (one species per ha) to 5 (three or more species per ha)") %>%
  mutate(score = case_when(abd_crop_species_per_ha>3~5,
                           abd_crop_species_per_ha>2~3.66,
                           abd_crop_species_per_ha>1~2.33,
                           abd_crop_species_per_ha==1~1,.default=NA))

d <- d %>% full_join(temp_kpi_calc[,c("kobo_farmer_id","country","indicator","subindicator","subindicator_label","score")], by = c("kobo_farmer_id", "country", "indicator", "subindicator","subindicator_label","score"), keep=NULL) %>%
  mutate(indicator_order = ifelse(indicator =="kpi6a_abd_species_diversity",6.1,indicator_order)) %>%
    mutate(module = ifelse(indicator == "kpi6a_abd_species_diversity","performance kpi",module)) 

ggplot(d %>% filter(indicator %in% c("kpi6a_abd_species_diversity")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi6a_abd_species_diversity")
