...
#country_short = "zwe"
#country.data.path = "C:/Users/sjones/CGIAR/Sanchez, Andrea Cecilia (Alliance Bioversity-CIAT) - HOLPA_data/Zimbabwe/zimbabwe_data_clean/zwe/"
#rm(d,country.data.path,country_short)
...
# (!!! fieldwork indicator extraction work in progress - not finished !!!)
  ...
  mutate(indicator = case_when(
    grepl("tree", name_question) ~ "biodiversity_trees_fw",
    grepl("biodiv", name_question) ~ "biodiversity_fw",
    grepl("1_201", name_question) ~ "biodiversity_landscape_fw", # landscape complexity
    grepl("1_202", name_question) ~ "biodiversity_landscape_fw", # landscape complexity
    grepl("1_203", name_question) ~ "biodiversity_landscape_fw", # landscape complexity
    grepl("1_301", name_question) ~ "crop_health_fw",
    grepl("1_302", name_question) ~ "crop_health_fw",
    grepl("1_303", name_question) ~ "crop_health_fw",
    grepl("1_111", name_question) ~ "soil_health_fw_prac", # practice
    grepl("1_112", name_question) ~ "soil_health_fw",
    grepl("1_113", name_question) ~ "soil_health_fw_prac", # practice
    grepl("1_114", name_question) ~ "soil_health_fw",
    grepl("1_115", name_question) ~ "soil_health_fw_prac", # practice
...
country_data <- ae %>% full_join(cn) %>% full_join(pf)  %>% full_join(fw) %>% full_join(ae_scores) %>% unique()
...
set_d <- function(country){
  ...
  d <- bur_data
  ref_values_country <- read_excel(paste0(ref.values.data.path,"HOLPA data post-processing.xlsx"),sheet=paste0("BUR","_inputs"))
...
d <- d %>%
  ...
  select(-c(label_score_agroecology_module))
...
d <- d %>% 
  mutate(subindicator = ifelse(label_question == "What percentage of the total crop production was lost or damaged in the last 12 months [add country meaning]","crop_loss",subindicator)) %>%
  mutate(subindicator = ifelse(label_question=="What was the main cause of crop loss or damage?", "crop_loss_cause", subindicator)) %>%
...
d <- d %>% 
  mutate(score = ifelse(indicator %in% c("kpi1_crop_health"), as.numeric(name_choice), score)) %>% 
  mutate(score = ifelse(indicator %in%  c("kpi1_crop_health") & score>100,NA,score)) %>%
...
temp_kpi_calc <- d %>% filter(indicator =="crop_health_fw") %>% filter(!is.na(name_choice)) %>%
  ...
  #select(kobo_farmer_id,country,subindicator,score) %>%
  dcast(kobo_farmer_id+country+label_question~subindicator,value.var="score") %>%
  
  # assign scores noting response 0 means "I am not sure, or the question is not applicable" 
  mutate_at(c("appearance" ,    "apperance", "disease"  ,      "enemy"  ,        "growth" ,        "management"  ,   "natural", "pest"  , "weed"  ),~na_if(.,0))  %>%
  rowwise() %>%
  mutate(score = median(c(appearance,    apperance, disease ,      enemy  ,   growth ,        management ,   natural, pest  , weed ),na.rm=T)) %>%
...
d <- d %>% full_join(temp_kpi_calc) %>%
  mutate(module = ifelse(indicator %in% c("kpi1b_crop_health_fieldwork"),"performance kpi",module))

ggplot(d %>% filter(indicator=="kpi1b_crop_health_fieldwork"),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi1b_crop_health_fieldwork")
...
#  summarise(nut_ref = mean(nut_ref_rainfed_clean,na.rm=TRUE)) %>%
#  mutate(nut_ref = ifelse(is.na(nut_ref),-9999,nut_ref))
#ref_nut_quantity_kg_ha <- ref_nut_quantity_kg_ha$nut_ref
...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  mutate(nut_quantity_kg_ha = ifelse(area_unit %in% c("acres","in acres","Acres"), nut_quantity_kg_ha * acre_to_hectare,nut_quantity_kg_ha))

temp_kpi_calc <- temp_kpi_calc %>%
  group_by(country) %>% mutate(med_nut_quantity_kg_ha = median(nut_quantity_kg_ha, na.rm=T)) %>% ungroup()  %>%
...
temp_kpi_calc <- d %>% filter(subindicator %in% c("bio_pollinators","bio_pests","bio_enemies","bio_mammals")) %>%
...
d <- d %>% full_join(temp_kpi_calc, by = c("kobo_farmer_id", "country", "indicator", "subindicator","subindicator_label","score"), keep=NULL) %>%
...
ggplot(d %>% filter(indicator %in% c("kpi5a_animal_diversity")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi5a_animal_diversity")
...
ggplot(d %>% filter(indicator %in% c("kpi5b_tree_diversity")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi5b_tree_diversity")
...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  mutate(abd_concentration = ifelse(abd_concentration > 1 & (abd_crop_area < abd_area_per_crop),(abd_area_per_crop/abd_area_per_crop_summed), abd_concentration)) %>%
  group_by_at(c("kobo_farmer_id","country")) %>%
  mutate(abd_concentration_sum = sum(abd_concentration)) %>% ungroup() %>%
  mutate(abd_concentration_score = ifelse(abd_concentration_sum<=1, 1 - abd_concentration_sum,0)*100) 
...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  mutate(abd_crop_richness_gap_score = (abd_crop_richness - abd_crop_richness_min)/(abd_crop_richness_max-abd_crop_richness_min)*100)
...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  mutate(indicator = ifelse(subindicator== "abd_concentration_score", "kpi6a_abd_concentration",
                            ifelse(subindicator == "abd_crop_richness_gap_score","kpi6a_abd_crop_richness_gap",NA)))

# Add this back into the main dataset
d <- d %>% full_join(temp_kpi_calc, by = c("kobo_farmer_id", "country", "indicator", "subindicator", "score"), keep=NULL) %>%
  mutate(subindicator_label = ifelse(indicator=="kpi6a_abd_concentration","Crop concentration index (0-100)",subindicator_label),
         subindicator_label = ifelse(indicator=="kpi6a_abd_crop_richness_gap","Crop richness gap (0-100)",subindicator_label),
         indicator_order = ifelse(indicator =="kpi6a_abd_concentration",6.11,indicator_order),
         indicator_order = ifelse(indicator =="kpi6a_abd_crop_richness_gap",6.12,indicator_order)) %>%
    mutate(module = ifelse(indicator %in% c("kpi6a_abd_concentration","kpi6a_abd_crop_richness_gap"),"performance kpi",module))

ggplot(d %>% filter(indicator %in% c("kpi6a_abd_concentration")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi6a_abd_concentration")

ggplot(d %>% filter(indicator %in% c("kpi6a_abd_crop_richness_gap")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi6a_abd_crop_richness_gap")
...
temp <- d %>% filter(subindicator %in% c( "abd_crop_local", "abd_livestock_local")) %>%
  select(subindicator,label_choice,name_choice, score) %>% unique()
...
d <- d %>% full_join(temp_kpi_calc, keep=NULL) %>%
  mutate(indicator_order = ifelse(indicator =="kpi6b_abd_variety_diversity",6.2,indicator_order)) %>%
    mutate(module = ifelse(indicator == "kpi6b_abd_variety_diversity","performance kpi",module)) 

ggplot(d %>% filter(indicator %in% c("kpi6b_abd_variety_diversity")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi6b_abd_variety_diversity")
...
d <- d %>%
  ...
  mutate(subindicator = ifelse(label_question == "How much land area is covered by natural and semi-natural vegetation on land owned, rented or used by the household: Any natural or semi-natural vegetation (bushland, fallow land, hedgerows, natural grassland, ponds, lakes, forest patches etc)","landscape_complexity",subindicator)) %>%
...
# assign scores
temp <- d %>% filter(subindicator %in% c( "landscape_complexity", "landscape_diversity_by_feature")) %>%
  select(subindicator,label_choice,name_choice, score) %>% unique()
arrange(temp,subindicator,score)
...
d <- d %>%
  ...
  mutate(score = case_when(subindicator == "landscape_diversity_by_feature" & name_choice %in% c("high")~5,
                           ...
                           subindicator == "landscape_diversity_by_feature" & name_choice %in% c("none")~1, .default=as.numeric(score)))

temp_kpi_calc <- d %>% filter(subindicator=="landscape_diversity_by_feature") %>%
  group_by_at(c("kobo_farmer_id","country")) %>%
...
d <- d %>% full_join(temp_kpi_calc, keep=NULL) %>%
  mutate(indicator_order = ifelse(indicator =="kpi7_landscape_complexity",7,indicator_order)) %>%
    mutate(module = ifelse(indicator == "kpi7_landscape_complexity","performance kpi",module)) 

ggplot(d %>% filter(indicator %in% c("kpi7_landscape_complexity")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi7_landscape_complexity")
...
d <- d %>% full_join(temp_kpi_calc, keep=NULL) %>%
  ...
  mutate(module = ifelse(indicator == "kpi8_climate_mitigation","performance kpi",module)) 

ggplot(d %>% filter(indicator %in% c("kpi8_climate_mitigation")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi8_climate_mitigation")
...
d <- d %>% full_join(temp_kpi_calc, keep=NULL) %>%
    mutate(module = ifelse(indicator %in% c("kpi9_avoided_water_stress"),"performance kpi",module)) 

ggplot(d %>% filter(indicator %in% c("kpi9_avoided_water_stress")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi9_avoided_water_stress")
...
d <- d %>% 
  ...
  mutate(score = ifelse(subindicator =="energy_source", as.numeric(name_choice), score)) 
...
d <- d %>% full_join(temp_kpi_calc, keep=NULL) %>%
  mutate(module = ifelse(indicator %in% c("kpi10_energy_use"),"performance kpi",module))
  
ggplot(d %>% filter(indicator %in% c("kpi10_energy_use")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi10_energy_use")
...
#ref_income <- ref_income$income_ref_clean
ref_income <- NA
...
ggplot(d %>% filter(indicator %in% c("kpi11_hh_income_v_mean")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi11_hh_income_v_mean")
...
ggplot(d %>% filter(indicator %in% c("kpi15_diet_diversity")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi15_diet_diversity")
...
ggplot(d %>% filter(indicator %in% c("kpi18_human_wellbeing")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi18 human wellbeing")

#### Graphical summary of indicators ####
#ggplot(d %>% filter(module %in% c("performance kpi")), aes(x=score,fill=theme))+
#  geom_histogram(binwidth=10)+scale_fill_viridis_d()+facet_wrap(~indicator, scale="free_x")+theme_bw()
```
...
indicator_scaling <- function(data, ind, method, min=NA,max=NA){
  ...
  # inverse scaling based on min and max values recorded in they survey
...
indicator_scaling(data = d, ind="kpi1b_crop_health_fieldwork",method="none")
...
indicator_scaling(data = d, ind="kpi4_nutrient_use", method="data_driven_inverse")
indicator_scaling(data = d, ind="kpi5a_animal_diversity", method="set",min=1,max=5)
indicator_scaling(data = d, ind="kpi5b_tree_diversity", method="set",min=1,max=5)
indicator_scaling(data = d, ind="kpi6a_abd_crop_richness_gap", method="none") 
indicator_scaling(data = d, ind="kpi6b_abd_variety_diversity", method="set" ,min=1,max=5) 
indicator_scaling(data = d, ind="kpi7_landscape_complexity ", method="set" ,min=1,max=5) 
indicator_scaling(data = d, ind="kpi8_climate_mitigation", method="set" ,min=1,max=5) 
indicator_scaling(data = d, ind="kpi9_avoided_water_stress", method="none")
indicator_scaling(data = d, ind="kpi10_energy_use", method="set" ,min=1,max=5) 
indicator_scaling(data = d, ind="kpi11_hh_income_v_mean", method="data_driven") 
...
indicator_scaling(data = d, ind="kpi15_diet_diversity", method="none") # CHECK
indicator_scaling(data = d, ind="kpi16_farmer_agency", method="set",min=0,max=10)
...
indicators_scaled <- indicators_scaled %>%
  mutate(theme_short = case_when(indicator %in% c("kpi1_crop_health",
                                                  "kpi1b_crop_health_fieldwork",
                                                  "kpi2_animal_health",
                                                  "kpi3_soil_health",
                                                  "kpi4_nutrient_use")~"AGR",
                                 
                                 indicator %in% c("kpi5a_animal_diversity",
                                                  "kpi5b_tree_diversity",
                                                  "kpi6a_abd_crop_richness_gap",
                                                  "kpi6b_abd_variety_diversity",
                                                  "kpi7_landscape_complexity",
                                                  "kpi8_climate_mitigation",
                                                  "kpi9_avoided_water_stress",
                                                  "kpi10_energy_use") ~ "ENV",
                                
                                 indicator %in% c("kpi11_hh_income_v_mean")~"ECO",
                                                                  
                                indicator %in% c("kpi15_diet_diversity",
                                                 "kpi16_farmer_agency",
                                                 "kpi17_land_tenure",
                                                 "kpi17b_land_tenure",
                                                 "kpi18_human_wellbeing")~"SOC",
...
# Attach overall agroecology scores 
d_ae_wide <- d %>%
  filter(indicator %in% c("score_ae_overall"))  %>% unique() %>%
  select(c("kobo_farmer_id", "indicator","score")) %>% 
  pivot_wider(names_from=c(indicator),
              values_from=c(score),
              #values_from=c(name_choice), # OR use score here if everything needs to be numeric
              names_sort=TRUE,
              #names_glue="{subindicator}_{.value}",
...
#indicators_scaled_wide <- indicators_scaled %>% full_join(d_cn_wide,by="kobo_farmer_id",relationship="many-to-many") %>%
#full_join(d_ae)

#indicators_scaled_wide <- indicators_scaled_wide %>% mutate(indicator = factor(indicator, levels=unique(indicator[order(indicator_order)])))

# Save data for re-use in next script
write.csv(indicators_scaled,paste0(zwe.data.path,"zwe_indicators.csv"),row.names=FALSE)
#write.csv(indicators_scaled_wide,paste0(zwe.data.path,"zwe_indicators_wide.csv"),row.names=FALSE)
