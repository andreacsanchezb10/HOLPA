...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  ungroup() %>%
    mutate_at(c("nut_chem_unit_conversion","nut_org_unit_conversion","nut_orgF_unit_conversion"), ~as.numeric())
...
indicator_scaling(kpi_scores, ind="kpi4_nutrient_use",method="set",min=0,max=1) 
...
indicator_scaling(kpi_scores, ind="kpi11_hh_income_v_mean",method="set",min=0,max=1) 
...
kpi_scores_final <- kpi_scores %>% left_join(indicators_scaled)  %>%
...
ggplot(ae_scores_cn, aes(y=score, x=factor(indicator,levels=unique(indicator[order(indicator_order)])), fill=District))+ 
  geom_boxplot()+ #geom_count()+ 
  ggtitle(paste0("Agroecology scores for ",country," (n = ",length(unique(ae_scores$kobo_farmer_id)), ")")) + 
  labs(x="")+ 
...
data <- kpi_scores_full %>%
  ...
  mutate(score_ae_overall_classes = case_when(score_ae_overall<2~"Mainly not AE",
                                              ...
                                              .default="Check")) %>%
     group_by(theme,indicator,indicator_order,score_ae_overall_classes) %>%
...
ggplot(data %>% filter(!is.na(kpi_score_scaled_mean)& !is.na(score_ae_overall_classes)), aes(x=kpi_score_scaled_mean,y=reorder(indicator,kpi_score_scaled_mean),fill=score_ae_overall_classes))+
...
ggplot(data %>% filter(!is.na(kpi_score_scaled_mean)& !is.na(score_ae_overall_classes)), aes(x=score_ae_overall_classes,y=reorder(indicator,kpi_score_scaled_mean),fill=kpi_score_scaled_mean ))+
...
ggplot(data %>% filter(!is.na(kpi_score_scaled_mean) & !is.na(score_ae_overall_classes)), aes(x=kpi_score_scaled_mean,y=reorder(indicator,kpi_score_scaled_mean),fill=score_ae_overall_classes))+
    geom_point(aes(colour=score_ae_overall_classes),alpha=0.7, size=4) + #size=n
  #geom_errorbar(aes(xmin=kpi_score_scaled_mean - kpi_score_scaled_sd,xmax=kpi_score_scaled_mean + kpi_score_scaled_sd))+
  #geom_smooth(aes(group=score_ae_overall_classes,colour=score_ae_overall_classes),method="loess",show.legend=FALSE)+
    ggtitle(paste0("KPIs for ",country))+labs(x="KPI score (scaled from 0-100)")+
    coord_cartesian(expand=c(0),clip="off")+
    scale_x_continuous(breaks=seq(0,100,20),limits=c(0,110))+
    scale_fill_manual(values=terrain.colors(5))+
