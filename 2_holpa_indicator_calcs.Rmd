...
# and set reference fertiliser use (kg/ha) used as the benchmark to determine the under or over use of fertilizer
...
temp_kpi_calc <- d %>% filter(indicator =="crop_health_fw") %>% filter(!is.na(name_choice)) %>%
  ...
  mutate(indicator = "kpi1b_crop_health_fieldwork",
         subindicator = "crop_health_socla",
...
temp_kpi_calc <- temp_kpi_calc %>%
  select(kobo_farmer_id,country, abd_concentration_score,abd_crop_richness_gap_score) %>% unique() %>%
...
d <- d %>%
  ...
  mutate(subindicator = ifelse(label_question %in% c("How would you describe the plant diversity (i.e., number of plant species) in: Bushland patches", "How would you describe the plant diversity (i.e., number of plant species) in: Fallow land which is unproductive for at least 1 year", "How would you describe the plant diversity (i.e., number of plant species) in: Hedgerows/Live fences", "How would you describe the plant diversity (i.e., number of plant species) in: Natural grassland",  "How would you describe the plant diversity (i.e., number of plant species) in: Remnant forest patches","How would you describe the plant diversity (i.e., number of plant species) in: Wetlands", "How would you describe the plant diversity (i.e., number of plant species) in: Woodlots"),"landscape_diversity_by_feature",subindicator))
...
d <- d %>%
  mutate(score = case_when(subindicator == "landscape_complexity" & label_choice =="51-100%"~5,
                           ...
                           subindicator == "landscape_complexity" & label_choice =="No presence of occurrence."~1,.default=as.numeric(score))) %>%
...
library(gridExtra)
ggplot(d %>% filter(module %in% c("performance kpi")),aes(x=score))+
  geom_histogram()+
  facet_wrap(~indicator, scale="free_x")+theme_bw()
```
...
indicator_scaling(data = d, ind="kpi6a_abd_crop_richness_gap", method="none") # CHECK
indicator_scaling(data = d, ind="kpi6b_abd_variety_diversity", method="none") # CHECK
indicator_scaling(data = d, ind="kpi7_landscape_complexity ", method="none") # CHECK
indicator_scaling(data = d, ind="kpi9_avoided_water_stress", method="none") # CHECK
#indicator_scaling(data = d, ind="kpi10", method="none") # CHECK
#indicator_scaling(data = d, ind="kpi11", method="none") # CHECK
...
#indicator_scaling(data = d, ind="kpi15", method="none") # CHECK
...
indicators_scaled <- indicators_scaled %>%
  mutate(theme_short = case_when(indicator %in% c("kpi1_crop_health",
                                 ...
                                 indicator %in% c("kpi5a_animal_diversity",
                                                  ...
                                                  "kpi9_avoided_water_stress")~"ENV",
                                indicator %in% c("kpi16_farmer_agency",
                                                 "kpi17_land_tenure",
                                                 "kpi17b_land_tenure",
