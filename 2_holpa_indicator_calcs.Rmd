...
ggplot(d %>% filter(module %in% c("agroecology") & !is.na(score)), aes(y=score, x=factor(indicator,levels=unique(indicator[order(indicator_order)])),fill=factor(indicator,levels=unique(indicator[order(indicator_order)]))))+ geom_boxplot()+geom_count()+ ggtitle(paste0("Agroecology scores for ",country)) + labs(x="")+ scale_fill_manual(values=terrain.colors(14),name="Indicators") +
...
temp_kpi_calc <- temp_kpi_calc %>%
  select(kobo_farmer_id,country,abd_crop_richness_gap_score) %>%
