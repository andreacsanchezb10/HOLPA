...
if(country=="peru"){
  ref_values_country <- ref_values_country %>%
    ...
    mutate(crops_names = ifelse(crops_names == "Maize","Corn",crops_names))
}
temp_kpi_calc2 <- temp_kpi_calc2 %>% 
  ...
  left_join(ref_values_country %>% rename(production_unit =`production unit`) %>%
              ...
              select(crops_names, production_unit,yield_conversion_unit_to_kg),by=c("abd_crop_name"="crops_names","production_unit" = "production_unit"))
...
temp_kpi_calc3 <- temp_kpi_calc2 %>%
  left_join(ref_values_country %>%
              mutate(crops_names = str_to_sentence(crops_names)) %>%
              select(crops_names, yield_ref_rainfed_clean) %>% unique(),by=c("abd_crop_name" = "crops_names")) %>%
...
temp_kpi_calc1 <- temp_kpi_calc %>% filter(subindicator %in% c("labour_number_workers_perm", "labour_hours_worker_perm", "labour_hours_hh_member_perm")) %>%
  ...
  mutate(across(-c(kobo_farmer_id,index),as.numeric)) %>%
  mutate(across(-c(kobo_farmer_id,index), ~ if_else(.x %in% c(9999, 999,-9999,-999,5555), NA_real_, .x)))
...
temp_kpi_calc2 <- temp_kpi_calc %>% filter(subindicator %in% c("labour_number_workers_seas", "labour_hours_worker_seas", "labour_number_workers_hh_seas","labour_hours_hh_member_seas")) %>%
  ...
  dcast(kobo_farmer_id+index~subindicator,value.var="name_choice", length),
...
temp_kpi_calc3 <- temp_kpi_calc1 %>% select(kobo_farmer_id,total_labour_hours_per_year_workers_perm, total_labour_hours_per_year_hh_perm) %>% unique() %>%
...
temp_kpi_calc <- temp_kpi_calc3 %>% select(kobo_farmer_id, total_labour_hours_per_year) %>%
  left_join(temp_kpi_calc4 %>% select(kobo_farmer_id, ag_land_area_ha) %>% unique()) %>%
    mutate(score = total_labour_hours_per_year/ag_land_area_ha, na.rm=TRUE) %>%
  select(kobo_farmer_id,score) %>%
  mutate(indicator = "kpi13_labour_productivity",
       subindicator = "labour_productivity",
       subindicator_label = "Labour in hours/yr/ha agricultural land",
       indicator_order = 13)

kpi_scores <- kpi_scores %>% full_join(temp_kpi_calc, keep=NULL) %>%
  mutate(module = ifelse(indicator %in% c("kpi13_labour_productivity"),"performance kpi",module)) %>%
  mutate(theme = ifelse(indicator %in% c("kpi13_labour_productivity"),"economic",theme)) 

kpi_histogram(kpi_scores,"kpi13_labour_productivity")
#kpi_all_boxplot(kpi_scores)
...
indicator_scaling(kpi_scores, ind="kpi12_yield_gap", method="set",min=0,max=99) 
indicator_scaling(kpi_scores, ind="kpi13_labour_productivity", method="data_driven_inverse") # used inverse so higher values are better, i.e. less labour per ha. This is hrs/yr/ha, which is an input not an output...could instead use income from agriculture divided by labour. 
indicator_scaling(kpi_scores, ind="kpi14a_climate_resilience", method="set",min=0,max=20)
indicator_scaling(kpi_scores, ind="kpi14b_climate_resilience",  method="set",min=1,max=5)
