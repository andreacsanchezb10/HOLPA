...
#----------------------- I NEED TO ADD A COLUMN WITH THE 18 performance theme names, separate from the list of indicators used to measure each theme, to be consistent with our framework figure. Replace 'theme' column with this? Add dimension column? We have 25 indicators not 18. Make some complementary not KPIs? ------------------
...
# CHECK IF WE CAN USE THE AREA RESPONSES:
#"In the past [localised description of 12 months], did you use any of these practices on #your cropland?"  "_3_3_3_1"
#"Provide the approximate area of land under this practice in hectares or acres:" #"_3_3_3_2_2"
...
# CHECK why Peru has not got these questions:
# _3_3_2_1_5_9_1: In the past [localised description of 12 months], what practices did you use to produce ${_3_4_3_1_3_calculate}?
...
ref_values_country <- ref_values_country %>% mutate(crops_names = ifelse(crops_names == "Maize","Corn",crops_names))
...
# calculate agricultural income output per labour input
temp_kpi_calc5 <- d %>% filter(subindicator =="income_amount") %>%
  mutate(score = ifelse(name_choice %in% c("9999","nd"),NA,name_choice)) %>%
  mutate(score = as.numeric(score)) %>%
  filter(!(is.na(score))) %>%
  filter(label_question %in% c("In the past 12 months, approximately how much income in [local currency] did the household make from: Crop production" ,"In the past 12 months, approximately how much income in [local currency] did the household make from: Livestock production"  ))  %>% group_by(kobo_farmer_id) %>% 
  summarise(income_ag_production = sum(score, na.rm=TRUE)) %>%
  ...
  mutate(indicator = "kpi13b_labour_productivity",
       ...
       subindicator_label = "Annual income from agricultural sources per hour of labour",
...
ABS <- temp_kpi_calc  %>% filter(subindicator %in% c("ABS_service_access", "ABS_service_access_unit", "ABS_service_access_transport"))  %>%  
  ...
  mutate(ABS_service_access_walking = case_when(ABS_service_access_transport %in% c("motobike","car")~ABS_service_access*10, # assumes car speed of 50km/h and walking speed of 5 km/h  
...
AC <- temp_kpi_calc  %>% filter(subindicator %in% c("AC_literacy")) %>%
  mutate(name_choice = ifelse(name_choice %in% c("777","999","5555","9999"),0,name_choice)) %>%
  ...
  by=c("kobo_farmer_id","country")) %>%
  full_join(
    temp_kpi_calc  %>% filter(subindicator %in% c("AC_income_on_food")) %>%
  ...
  by=c("kobo_farmer_id","country"))   %>%
  group_by(kobo_farmer_id,country) %>%
  summarise(score_AC = sum(AC_literacy_score,AC_highest_school_score,AC_training_hh,AC_training_score, AC_income_sources_score, AC_income_on_food_scores, AC_access_to_credit_score, AC_credit_repayments_score, AC_ag_insurance_score, AC_ag_subsidies_score, na.rm=TRUE)/10)
...
indicator_scaling <- function(data, ind, method, min=NA,max=NA,ind_group){
  ...
  data_subset <- data_subset %>% mutate(ind_group = ind_group) %>%
    select(c("kobo_farmer_id","theme","ind_group", "indicator","indicator_order","subindicator","subindicator_label", "score", "score_scaled"))
...
indicator_scaling(kpi_scores, ind="kpi1_crop_health",method="none", ind_group="Crop health")
indicator_scaling(kpi_scores, ind="kpi1b_crop_health_fieldwork",method="set", min=1,max=5, ind_group="Crop health")
indicator_scaling(kpi_scores, ind="kpi2_animal_health", method="set", min=1,max=5, ind_group="Animal health")
indicator_scaling(kpi_scores, ind="kpi3_soil_health",method="set",min=1,max=5, ind_group="Soil health")
indicator_scaling(kpi_scores, ind="kpi4_nutrient_use",method="set",min=0.5,max=2, ind_group="Nutrient use")
indicator_scaling(kpi_scores, ind="kpi5a_animal_diversity", method="set",min=1,max=5, ind_group="Biodiversity")
indicator_scaling(kpi_scores, ind="kpi5b_tree_diversity", method="set",min=1,max=5, ind_group="Biodiversity")
indicator_scaling(kpi_scores, ind="kpi6a_abd_crop_richness_index", method="none", ind_group="Agrobiodiversity") 
indicator_scaling(kpi_scores, ind="kpi6b_abd_variety_diversity", method="set" ,min=1,max=5, ind_group="Agrobiodiversity") 
indicator_scaling(kpi_scores, ind="kpi7_landscape_complexity", method="set" ,min=1,max=5, ind_group="Landscape complexity")
indicator_scaling(kpi_scores, ind="kpi8_climate_mitigation", method="set" ,min=1,max=5, ind_group="Climate mitigation") 
indicator_scaling(kpi_scores, ind="kpi9_avoided_water_stress", method="none", ind_group="Water use")
indicator_scaling(kpi_scores, ind="kpi10_energy_use", method="set" ,min=1,max=5, ind_group="Energy use")
indicator_scaling(kpi_scores, ind="kpi11_hh_income_v_mean", method="set", min=0.5, max=2, ind_group="Income")
indicator_scaling(kpi_scores, ind="kpi11b_hh_income_stability",method="set",min=1,max=5, ind_group="Income") 
indicator_scaling(kpi_scores, ind="kpi11c_hh_income_v_expenditures", method="set",min=0,max=1, ind_group="Income") 
indicator_scaling(kpi_scores, ind="kpi11d_hh_income_sufficiency",method="set",min=1,max=5, ind_group="Income")  
indicator_scaling(kpi_scores, ind="kpi12_yield_gap", method="set",min=0,max=99, ind_group="Agricultural productivity")  
indicator_scaling(kpi_scores, ind="kpi13a_labour_input", method="data_driven_inverse", ind_group="Labour") # used inverse so higher values are better, i.e. less labour per ha. Need to get min and max across all countries to allow for better comparisons.
indicator_scaling(kpi_scores, ind="kpi13b_labour_productivity", method="data_driven", ind_group="Labour") # this is local currency/hrs of labour. Need to convert currency to USD and then set min and max productivity based on this, to allow for better comparisons across all countries. 
indicator_scaling(kpi_scores, ind="kpi14a_climate_resilience", method="set",min=0,max=20, ind_group="Climate resilience")
indicator_scaling(kpi_scores, ind="kpi14b_climate_resilience",  method="set",min=1,max=5, ind_group="Climate resilience")
indicator_scaling(kpi_scores, ind="kpi15_diet_diversity", method="set",min=0,max=10, ind_group="Diet diversity")
indicator_scaling(kpi_scores, ind="kpi16_farmer_agency", method="set",min=1,max=5, ind_group="Farmer agency")
indicator_scaling(kpi_scores, ind="kpi17a_land_tenure", method="set",min=1,max=5, ind_group="Land tenure")
indicator_scaling(kpi_scores, ind="kpi17b_land_tenure", method="none", ind_group="Land tenure")
indicator_scaling(kpi_scores, ind="kpi18_human_wellbeing", method="set",min=1, max=5, ind_group="Human wellbeing")
...
kpi_scores_final <- kpi_scores %>% left_join(indicators_scaled %>% select(-score), relationship="many-to-many")  %>%
  ...
  mutate(kpi_label= paste0(gsub("(.+?)(\\_.*)","\\1", indicator),": ",subindicator_label)) %>%
     mutate(kpi_label_short = case_when(
       ...
       indicator %in% c("kpi12_yield_gap" ) ~ "kpi12 yield gap (%)",
       indicator %in% c("kpi13_labour_productivity" ) ~ "kpi13 labour input (hrs/yr/ha)",
       indicator %in% c("kpi14a_climate_resilience" ) ~ "kpi14a climate resilience (RIMA)", 
       indicator %in% c("kpi14b_climate_resilience" ) ~ "kpi14b recovery after shocks (qual)", 
