...
#global.data.path <- "C:/Users/sjones/CGIAR/Sanchez, Andrea Cecilia (Alliance Bioversity-CIAT) - HOLPA_data/"

# import cleaned data
zwe_ae <- read_csv(paste0(zwe.data.path,"zwe_agroecology_format.csv"),show_col_types = FALSE)
zwe_cn <- read_csv(paste0(zwe.data.path,"zwe_context_format.csv"),show_col_types = FALSE)
zwe_pf <- read_csv(paste0(zwe.data.path,"zwe_performance_format.csv"),show_col_types = FALSE)
zwe_fw <- read_csv(paste0(zwe.data.path,"zwe_fieldwork_format.csv"),show_col_types = FALSE)
zwe_ae_scores <- read_csv(paste0(zwe.data.path,"zwe_agroecology_score.csv"),show_col_types = FALSE)

# reshape and classify fieldwork data to enable join with household
zwe_fw <- melt(zwe_fw[,c(1,2,33:length(zwe_fw))],id.vars=c("kobo_farmer_id","household_id","country"),variable.name="name_question",value.name="name_choice")

zwe_fw <- zwe_fw %>%
  mutate(indicator = case_when(
    grepl("tree", name_question) ~ "biodiversity",
    grepl("biodiv", name_question) ~ "biodiversity",
    grepl("1_201", name_question) ~ "biodiversity", # landscape complexity
    grepl("1_202", name_question) ~ "biodiversity", # landscape complexity
    grepl("1_203", name_question) ~ "biodiversity", # landscape complexity
    grepl("1_301", name_question) ~ "crop health",
    grepl("1_302", name_question) ~ "crop health",
    grepl("1_303", name_question) ~ "crop health",
    grepl("1_111", name_question) ~ "soil health", # practice
    grepl("1_112", name_question) ~ "soil health",
    grepl("1_113", name_question) ~ "soil health", # practice
    grepl("1_114", name_question) ~ "soil health",
    grepl("1_115", name_question) ~ "soil health", # practice
    grepl("1_116", name_question) ~ "soil health",
    .default = NA)) %>%
...
zwe_data <- zwe_ae %>% full_join(zwe_cn) %>% full_join(zwe_pf) %>% full_join(zwe_fw)  %>% left_join(zwe_ae_scores) %>% unique()
...
#data_output <- d[0,]
data_output <- data.frame("kobo_farmer_id"=as.numeric(NA), "indicator"=as.character(NA),score_scaled=as.numeric(NA)) 
...
indicator_scaling <- function(data, ind, method, min,max){
  ...
  data_subset <- data_subset %>% select(c("kobo_farmer_id","indicator", "score_scaled"))
  data_output <- data_output %>% rbind(data_subset)
  assign("data_output",data_output,envir = globalenv())
...
indicator_scaling(data = d, ind="kpi4_nutrient_use", method="data_driven", min=NA,max=NA)
...
data_output <- data_output %>%
  mutate(theme_short = ifelse(indicator %in% c("kpi1_crop_health","kpi2_animal_health","kpi3_soil_health","kpi4_nutrient_use"),"AGR",
                        ifelse(indicator %in% (c("kpi16","kpi17","kpi17b","kpi18_human_wellbeing")),"SOC","CHECK")))

ggplot(data_output %>% filter(!is.na(score_scaled)),
...
d <- d %>%
  ...
  mutate(label_choice_short = ifelse(name_choice=="0" & subindicator == "Ae_understanding","No",
                                     ifelse(name_choice=="1" & subindicator == "Ae_understanding","Some",
                                            ifelse(name_choice=="2" & subindicator == "Ae_understanding","Yes",label_choice)))) %>%
