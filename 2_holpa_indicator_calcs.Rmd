...
In this section, the user needs to specify which country to analyse, where the cleaned HOLPA survey data are saved, and any country/landscape specific conversion parameters.
...
#### Performance KPI4 nutrient use NEEDS CONVERTING TO NUTRIENT GAP ####
#sort(unique(d$indicator))
...
d <- d %>% full_join(temp_kpi_calc, by = c("kobo_farmer_id", "country", "indicator", "subindicator", "score"), keep=NULL) %>%
  mutate(subindicator_label = ifelse(indicator %in% c("kpi4_nutrient_use"),"Fertilizer use (kg/ha)",subindicator_label),
...
d <- d %>% full_join(temp_kpi_calc, keep=NULL) %>%
    mutate(module = ifelse(indicator %in% c("kpi9_water_stress"),"performance kpi",module)) 

ggplot(d %>% filter(indicator %in% c("kpi9_water_stress")),aes(x=score))+
  geom_histogram()+
  ggtitle("kpi9_water_stress")
...
#d <- d %>% filter(!(is.na(score) & is.na(name_choice)))
...
d_wide <- d %>% filter(!(module=="context")) %>%
  full_join(d_cn_wide,by="kobo_farmer_id",relationship="many-to-many") 
