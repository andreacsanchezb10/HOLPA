...
indicator_scaling <- function(data, ind, method, min=NA,max=NA){
  ...
  data_subset <- data_subset %>% select(c("kobo_farmer_id","indicator","subindicator","subindicator_label", "score", "score_scaled"))
...
indicators_scaled <- indicators_scaled %>%
  ...
  mutate(indicator = factor(indicator, levels=unique(indicator)[order(theme_short)]))
