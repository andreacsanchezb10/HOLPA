...
nut_conversion_tractor_to_kg <- 2000 # CHECK THIS
nut_conversion_quintaux_to_kg <- 100
...
kpi_histogram <- function(data, kpi){
  g <- ggplot(data %>% filter(indicator %in% c(kpi)),aes(x=score))+
  geom_histogram()+
  ggtitle(kpi)
  print(g)
}
...
kpi_all_boxplot <- function(data){
  g <- ggplot(data %>% filter(module %in% c("performance kpi")), aes(y=score,x=factor(indicator,levels=unique(indicator[order(indicator_order)])), fill=theme))+ # geom_histogram()+  
  ggtitle(paste0("KPIs for ",country))+labs(x="")+
  geom_boxplot()+  geom_count()+ scale_fill_manual(values=terrain.colors(5))+
  facet_wrap(~indicator, scale="free")+theme_bw()+theme(axis.text.x = element_blank())
  print(g)
}
...
ggplot(d %>% filter(module %in% c("agroecology")), aes(y=score,x=factor(indicator,levels=unique(indicator[order(indicator_order)])),fill=factor(indicator,levels=unique(indicator[order(indicator_order)]))))+ geom_boxplot()+geom_count()+ ggtitle(paste0("Agroecology scores for ",country)) + labs(x="")+ scale_fill_manual(values=terrain.colors(14)) + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))
...
temp_kpi_calc <- d %>%
  ...
  mutate(score = as.numeric(score)) %>%
  mutate(score = ifelse(indicator %in%  c("kpi1_crop_health") & score>100,100,score)) %>%
...
temp_kpi_calc <- temp_kpi_calc %>%
  ...
  mutate(nut_chem_lt_area_converted = ifelse(nut_chem_quantity>0 & nut_chem_unit %in% c("Liters","Litres" ,"litres","liters"), nut_chem_quantity/nut_chem_area* nut_conversion_l_to_kg, 0)) %>%
   ...
   # Tonnes
  mutate(nut_chem_tn_area_converted = ifelse(nut_chem_quantity>0 & nut_chem_unit %in% c("ton","tonnes","Ton","Tonnes"), (nut_chem_quantity/nut_chem_area)/1000, 0)) %>%
  
     # Tractor
  mutate(nut_chem_tn_area_converted = ifelse(nut_chem_quantity>0 & nut_chem_unit %in% c("Tractor","Tracteur","tractor","tracteur"), (nut_chem_quantity/nut_chem_area)*nut_conversion_tractor_to_kg, 0)) %>%
  
       # Quintaux
  mutate(nut_chem_tn_area_converted = ifelse(nut_chem_quantity>0 & nut_chem_unit %in% c("Quintaux","quintaux"), (nut_chem_quantity/nut_chem_area)*nut_conversion_quintaux_to_kg, 0)) %>%
...
kpi_scores <- kpi_scores %>% full_join(temp_kpi_calc, by = c("kobo_farmer_id", "country", "indicator", "subindicator", "score"), keep=NULL) %>%
  mutate(subindicator_label = ifelse(indicator=="kpi6a_abd_concentration","Crop concentration index (0-100)",subindicator_label),
    ...
    mutate(module = ifelse(indicator %in% c("kpi6a_abd_concentration","kpi6a_abd_crop_richness_gap"),"performance kpi",module)) %>% mutate(theme = ifelse(indicator %in% c("kpi6a_abd_concentration","kpi6a_abd_crop_richness_gap"),"environmental",theme))

kpi_histogram(kpi_scores,"kpi6a_abd_concentration")
kpi_histogram(kpi_scores,"kpi6a_abd_crop_richness_gap")
kpi_all_boxplot(kpi_scores)
...
kpi_scores <- kpi_scores %>% full_join(temp_kpi_calc, by = c("kobo_farmer_id", "country", "indicator", "subindicator", "score"), keep=NULL) %>%
  ...
  mutate(theme = ifelse(indicator %in% c("kpi17_land_tenure","kpi17b_land_tenure"),"social",theme)) 

kpi_histogram(kpi_scores,"kpi17_land_tenure")
...
indicator_scaling(kpi_scores, ind="kpi1b_crop_health_fieldwork",method="none")
...
indicator_scaling(kpi_scores, ind="kpi4_nutrient_use", method="data_driven_inverse")
...
indicator_scaling(kpi_scores, ind="kpi6a_abd_crop_richness_gap", method="none") 
...
indicator_scaling(kpi_scores, ind="kpi11_hh_income_v_mean", method="data_driven") 
...
indicator_scaling(kpi_scores, ind="kpi15_diet_diversity", method="none") # CHECK
indicator_scaling(kpi_scores, ind="kpi16_farmer_agency", method="set",min=0,max=10)
indicator_scaling(kpi_scores, ind="kpi17_land_tenure", method="set",min=1,max=5)
...
kpi_scores_final <- kpi_scores %>% left_join(indicators_scaled) %>% select(c("kobo_farmer_id","country","module", "theme", "indicator","indicator_order","subindicator","subindicator_label", "score", "score_scaled"))

# Save data for re-use in next script
export_kpi_scores <- function(country){
  if(country == "burkina"){
  write.csv(kpi_scores_final,paste0(bur.data.path,"bur_kpi_scores.csv"),row.names=FALSE) }
  if(country=="india"){
...
write.csv(kpi_scores_final,paste0(per.data.path,"per_kpi_scores.csv"),row.names=FALSE)}
  ...
  if(country=="tunisia"){write.csv(kpi_scores_final,paste0(tun.data.path,"tun_kpi_scores.csv"),row.names=FALSE)
  }
  if(country == "zimbabwe"){
