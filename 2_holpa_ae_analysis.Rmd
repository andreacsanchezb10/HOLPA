...
zwe_ae <- read.csv("agroecology_zimbabwe.csv",header=TRUE)
...
d <- zwe_ae %>% filter(X=="") %>%
  ...
  mutate(indicator_order = as.numeric(indicator_order),
         score_final = as.numeric(score_final)) %>%
...
ggplot(d, aes(x=indicator,y=score_final,fill=indicator))+
  geom_boxplot()+
  ggtitle(paste0("Agroecology adherence across n= ",length(unique(d$kobo_id))," farm-households"))+
